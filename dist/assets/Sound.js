...
(function (Ω) {
  ...
  Sound = Ω.Class.extend({
    ...
    init: function init(path, volume, loop) {
      var audio, resolve, onload;
      ...
      if (!sounds[path]) {
        ...
        audio._loaded = false; // Fixme: crazyies in firefox... fires twice?

        audio.addEventListener("canplaythrough", onload, false);

        var prog = function prog() {
  ...
  Sound._reset = function () {
    var path, sound; // Should check for canplaythrough before doing anything...
