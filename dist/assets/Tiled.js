...
(function (Ω) {
  ...
  var Tiled = Ω.Class.extend({
    ...
    tileH: null,
    layers: null,
    init: function init(file, onload) {
      var self = this;
      this.layers = [];
      this.onload = onload; // TODO: make sure not cached!
    ...
    layer: function layer(name) {
      var layer = Ω.utils.getByKeyValue(this.layers, "name", name),
          tileH = this.tileH,
          adjustYPos = function adjustYPos(obj) {
    ...
    processLevel: function processLevel(json) {
      this.raw = json;
      this.w = json.width;
      this.h = json.height;
