...
(function (Ω) {
  ...
  var Strip, RayCastMap;
  RayCastMap = Ω.Map.extend({
    strips: null,
    init: function init(sheet, data, stripWidth, fov) {
    ...
    setStripWidth: function setStripWidth(stripWidth) {
    ...
    tick: function tick(entities, player) {
    ...
    castRays: function castRays(entities, p) {
      var i,
          strip,
          rayPos,
          rayDist,
      ...
      for (i = 0; i < this.numRays; i++) {
        strip = this.strips[i];
        strip.depth = 100; // where on the screen does ray go through?
        ...
        if (hit) {
          ...
          texY = hit.shaded ? 32 : 0; // Set the ray strip
          // TODO: should be "this.stripWidth", or "width"?

          strip.set(i * this.stripWidth, top, this.stripWidth, height, hit.dist, texX, texY);
        }
      }
    },
    render: function render(gfx, entities, player) {
      var c = gfx.ctx;
  ...
  Strip = Ω.Class.extend({
    ...
    set: function set(x, y, w, h, depth, texX, texY) {
      this.x = x;
      this.y = y;
      this.w = w;
      this.h = h;
    ...
    render: function render(gfx) {
      ...
      gfx.ctx.drawImage(this.map.sheet, this.texX, this.texY, 1, 32, this.x, this.y, this.w, this.h);
    }
  });
