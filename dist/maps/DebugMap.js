...
(function (Ω) {
  ...
  var DebugMap = Ω.Map.extend({
    init: function init(tileW, tileH, xTiles, yTiles, cells, walkable, seed) {
      ...
      for (var i = 0; i < numPix; i++) {
        var row = i / data.width | 0,
            ...
            color = row / tileH + 1 + col * 3 + off + noise / 10 | 0; // Remove the edges, for some roundiness.
      ...
      this._super(new Ω.SpriteSheet(ctx.canvas, tileW, tileH), cells, walkable);
    },
    dump: function dump() {
